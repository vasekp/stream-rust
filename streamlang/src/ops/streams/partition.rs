...
fn eval_partition(node: &Node, env: &Env) -> SResult<Item> {
    ...
    let [size] = &node.args[..] else {
        return Err(StreamError::usage(&node.head));
    };
    let size = size.as_num()?.try_cast_within(1usize..)?;
...
impl<I: ItemType> Stream for Partition<I> {
    fn to_iter(self: Rc<Self>) -> Box<dyn SIterator> {
        ...
        PartitionIter{size: self.size, iter, node: self}.wrap()
    }

    fn len(&self) -> Length {
        self.source.len().map(|len| (len + self.size - 1) / self.size)
    }
}

struct PartitionIter<I: ItemType> {
    node: Rc<Partition<I>>,
    iter: Box<dyn SIterator<I>>,
    size: usize,
}

impl<I: ItemType> PreIterator for PartitionIter<I> {
